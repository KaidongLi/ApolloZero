...
class ClassificationModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # out is B x C x W x H, with C = n_classes + n_anchors  Now this should be (n_classes + n_locscore) + n_anchors
...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes, block, layers):
        ...
        self.locscoreModel = LocscoreModel(256)           # wenchi
        ...
        self.locscoreModel.output.weight.data.fill_(0)         # wenchi
        self.locscoreModel.output.bias.data.fill_(0)           # wenchi
    ...
    def forward(self, inputs):
        ...
        locscore = torch.cat([self.locscoreModel(feature) for feature in features], dim=1)            # wenchi
        ...
        if self.training:
            ...
            #locscore_over_thresh = (locscore>0.05)[0, :, 0]
            merged_scores = scores * locscore
            # for test, kaidong
            scores_over_thresh = (merged_scores>0.05)[0, :, 0]
            #scores_over_thresh = (merged_scores>=0.0005)[0, :, 0]

            # for test, kaidong
            #print('model', 'score dim', scores.shape)
            #print('model', 'locsc dim', locscores.shape)
            #print('model', 'mlocs dim', merged_scores.shape)
            #print('model', 'sc234', scores[:, 50:52, :])
            #print('model', 'ls234', locscore[:, 50:52, :])
            #print('model', 'ms234', merged_scores[:, 50:52, :])
            #print('model', 'lsmax', torch.max(locscore))
            #print('model', 'msmax', torch.max(merged_scores))
            #scores = torch.max(locscore, dim=2, keepdim=True)[0]
